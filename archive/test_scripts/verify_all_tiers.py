"""
Verification script to check all 4 generated presentations
Confirms: slide counts, no "nan%" values, chart/table presence
"""
...
def verify_presentation(ppt_path, expected_slides, tier_name):
    ...
    for slide_idx, slide in enumerate(prs.slides, 1):
        # Check text for "nan" (excluding "Financial", "Analysis", etc.)
        for shape in slide.shapes:
            if shape.has_text_frame:
                text = shape.text.lower()
                # Check for "nan" but exclude false positives like "financial", "analysis"
...
def main():
    ...
    print("\n" + "="*80)
    print("🎯 VERIFYING ALL GENERATED PRESENTATIONS")
    print("="*80)
    ...
    presentations = [
        {
            'path': os.path.join(base_path, 'tech_stocks_basic_tier.pptx'),
            'expected_slides': 7,
            'tier': 'BASIC TIER (1 AI feature: Executive Summary)'
        },
        {
            'path': os.path.join(base_path, 'tech_stocks_pro_tier.pptx'),
            'expected_slides': 7,
            'tier': 'PRO TIER (2 AI features: Summary + Category Insights)'
        },
        {
            'path': os.path.join(base_path, 'tech_stocks_ai_pro_tier.pptx'),
            'expected_slides': 8,
            'tier': 'AI_PRO TIER (3 AI features: Full Suite)'
        },
        {
            'path': os.path.join(base_path, 'GOOGL_Financial_Analysis.pptx'),
            'expected_slides': 8,
            'tier': 'GOOGL FINANCIAL ANALYSIS (AI_PRO)'
        }
    ]
    ...
    print("\n" + "="*80)
    if all_passed:
        print("🎉 ALL PRESENTATIONS VERIFIED SUCCESSFULLY!")
        print("✅ No 'nan%' values found")
        print("✅ All slide counts correct")
        print("✅ All charts have legends")
        print("\n📦 DELIVERABLES READY:")
        print("   1. Tech_Stocks_BASIC_Tier.pptx (7 slides, 1 AI feature)")
        print("   2. Tech_Stocks_PRO_Tier.pptx (7 slides, 2 AI features)")
        print("   3. Tech_Stocks_AI_PRO_Tier.pptx (8 slides, 3 AI features)")
        print("   4. GOOGL_Financial_Analysis.pptx (8 slides, GOOGL-focused)")
    else:
